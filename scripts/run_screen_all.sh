...
OUTPUT="${DOUBLE_ATTENTION_OUTPUT:-$ROOT/runs/screen}"

# Triton compiles a tiny Python extension on first use.  The WSL setup used by
# this project keeps Python development headers in the user's home directory
# so no administrator access is required.
...
for variant in "${variants[@]}"; do
  log="$OUTPUT/${variant}_screen.log"
  echo "[$(date --iso-8601=seconds)] starting $variant" | tee -a "$log"
